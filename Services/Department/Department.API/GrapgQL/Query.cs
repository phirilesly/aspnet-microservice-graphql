...
namespace StudentDepartment.API.GrapgQL
{
    public class Query
    {
        ...
        public Task<List<IDepartmentEntity>> GetDepartmentsAsync(IResolverContext ctx) => ctx.Service<IDemoGraphQLQueryHandler>().GetDepartments();



        public Task<List<ILecturerEntity>> GetLecturersAsync(IResolverContext ctx) => ctx.Service<IDemoGraphQLQueryHandler>().GetLecturers();
